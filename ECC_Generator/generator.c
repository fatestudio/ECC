...
int NUM; //54  // number of bits of our desired prime limit
...
int tot = 0;	// shared generated value!
pthread_mutex_t print_mutex; 	// mutex for printing
pthread_mutex_t tot_mutex;		// mutex for tot
...
int main(int argc, char *argv[]){
	...
	if(argc != 2){
		printf("Please run like this: ./generator number_of_bits_for_prime > ecc$(NUM).txt\n");
		exit(-1);
	}
	NUM = atoi(argv[1]);

	for(i = 0; i < NUM_THREADS; i++){
		rc = pthread_create(&threads[i], NULL, generator, (void *)i);
		if(rc){
